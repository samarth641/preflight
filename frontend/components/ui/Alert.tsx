...
const config: Record<Severity, { icon: typeof Info; color: string; bg: string; border: string }> = {
  info: { icon: Info, color: "var(--info)", bg: "rgba(6, 182, 212, 0.06)", border: "rgba(6, 182, 212, 0.25)" },
  warning: { icon: AlertTriangle, color: "var(--warning)", bg: "rgba(245, 158, 11, 0.06)", border: "rgba(245, 158, 11, 0.25)" },
  error: { icon: XCircle, color: "var(--danger)", bg: "rgba(239, 68, 68, 0.06)", border: "rgba(239, 68, 68, 0.25)" },
  success: { icon: CheckCircle2, color: "var(--success)", bg: "rgba(34, 197, 94, 0.06)", border: "rgba(34, 197, 94, 0.25)" },
}
  ...
  return (
    <div className={`flex gap-3 p-4 rounded-lg border ${className}`} style={{ background: bg, borderColor: border }}>
      ...
      <div className="flex-1">
        {title && <div className="text-sm font-semibold mb-1" style={{ color }}>{title}</div>}
